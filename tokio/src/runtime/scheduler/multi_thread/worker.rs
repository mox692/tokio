...
use crate::runtime::task::trace::Trace;
...
pub(crate) enum WorkerEvent {
    ...
    /// Spawn new task
    RunTask(u64, String), // task id, stack trace
    ...
    fn run_task(&self, task: Notified, mut core: Box<Core>) -> RunResult {
        ...
        *self.core.borrow_mut() = Some(core);
        ...
        coop::budget(|| {
            ...
            loop {
                // capture stack-trace generated by `task.run()`
                ...
                // worker logging
                core = self.worker.worker_log_with_elapsed(
                    core,
                    WorkerEvent::RunTask(task_id, bt.to_string()),
                    time,
                );
                ...
                *self.core.borrow_mut() = Some(core);
                current_task = self.worker.handle.shared.owned.assert_owner(task);
            }
        })
    }
...
impl Worker {
    ...
    fn worker_log(&self, core: Box<Core>, event: WorkerEvent) -> Box<Core> {
        ...
        let json_log = match &event {
            WorkerEvent::RunTask(_, trace) => format!(
                "{{ \"time\": {:>10}, \"id\": {:>4}, \"type\": \"{}\", \"trace\": `{}` }},",
                time, self.index, &event, trace
            ),
            _ => format!(
                "{{ \"time\": {:>10}, \"id\": {:>4}, \"type\": \"{}\" }},",
